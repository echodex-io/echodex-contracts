...
struct PoolInfo {
  uint256 allocPoint;
  contract IEchodexV3Pool v3Pool;
  address token0;
  address token1;
  uint24 fee;
...
```solidity
event AddPool(uint256 pid, uint256 allocPoint, contract IEchodexV3Pool v3Pool, contract ILMPool lmPool)
```
...
```solidity
function add(uint256 _allocPoint, contract IEchodexV3Pool _v3Pool, bool _withUpdate) external
```
...
| _v3Pool | contract IEchodexV3Pool | Address of the V3 pool. |
