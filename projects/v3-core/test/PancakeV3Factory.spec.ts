...
describe('EchodexV3Factory', () => {
  let wallet: Wallet, other: Wallet

  let deployer: EchodexV3PoolDeployer
  ...
  const fixture = async () => {
    ...
    const deployer_ = (await deployerFactory.deploy()) as EchodexV3PoolDeployer
    const factory_ = (await factoryFactory.deploy(deployer_.address)) as EchodexV3Factory

    await deployer_.setFactoryAddress(factory_.address)
    return [factory_, deployer_] as [EchodexV3Factory, EchodexV3PoolDeployer]
  }
  ...
  before('load pool bytecode', async () => {
    poolBytecode = (await ethers.getContractFactory('EchodexV3Pool')).bytecode
  })
  ...
  async function createAndCheckPool(
    ...
    const poolContractFactory = await ethers.getContractFactory('EchodexV3Pool')
