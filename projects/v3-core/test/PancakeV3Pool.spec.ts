...
import { TestEchodexV3SwapPay } from '../typechain-types/contracts/test/TestEchodexV3SwapPay'
...
import { TestEchodexV3ReentrantCallee } from '../typechain-types/contracts/test/TestEchodexV3ReentrantCallee'
...
describe('EchodexV3Pool', () => {
  ...
  let factory: EchodexV3Factory
  let pool: MockTimeEchodexV3Pool

  let swapTarget: TestEchodexV3Callee
  ...
  async function initializeAtZeroTick(pool: MockTimeEchodexV3Pool): Promise<void> {
    await pool.initialize(encodePriceSqrt(1, 1))
  ...
  describe('#lock', () => {
    ...
    it('cannot reenter from swap callback', async () => {
      const reentrant = (await (
        await ethers.getContractFactory('TestEchodexV3ReentrantCallee')
      ).deploy()) as TestEchodexV3ReentrantCallee
  ...
  describe('swap underpayment tests', () => {
    let underpay: TestEchodexV3SwapPay
    beforeEach('deploy swap test', async () => {
      const underpayFactory = await ethers.getContractFactory('TestEchodexV3SwapPay')
      underpay = (await underpayFactory.deploy()) as TestEchodexV3SwapPay
