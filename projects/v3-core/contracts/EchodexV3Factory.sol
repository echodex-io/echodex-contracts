...
/// @title Canonical EchodexSwap V3 factory
/// @notice Deploys EchodexSwap V3 pools and manages ownership and control over pool protocol fees
contract EchodexV3Factory is IEchodexV3Factory {
    /// @inheritdoc IEchodexV3Factory
    ...
    function createPool(
        ...
        pool = IEchodexV3PoolDeployer(poolDeployer).deploy(address(this), token0, token1, fee, tickSpacing);
    ...
    function setFeeProtocol(address pool, uint32 feeProtocol0, uint32 feeProtocol1) external override onlyOwner {
        IEchodexV3Pool(pool).setFeeProtocol(feeProtocol0, feeProtocol1);
    }
    ...
    function collectProtocol(
        ...
        return IEchodexV3Pool(pool).collectProtocol(recipient, amount0Requested, amount1Requested);
    }

    function setLmPool(address pool, address lmPool) external override onlyOwnerOrLmPoolDeployer {
        IEchodexV3Pool(pool).setLmPool(lmPool);
    }
}
