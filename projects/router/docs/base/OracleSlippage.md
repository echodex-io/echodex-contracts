...
```solidity
function getBlockStartingAndCurrentTick(contract IEchodexV3Pool pool) internal view returns (int24 blockStartingTick, int24 currentTick)
```
...
```solidity
function getPoolAddress(address tokenA, address tokenB, uint24 fee) internal view virtual returns (contract IEchodexV3Pool pool)
```
