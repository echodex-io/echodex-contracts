...
import '@echodex/v3-core/contracts/interfaces/callback/IEchodexV3FlashCallback.sol';
...
/// @notice An example contract using the EchodexSwap V3 flash function
contract PairFlash is IEchodexV3FlashCallback, PeripheryPayments {
    ...
    /// @notice Calls the pools flash function with data needed in `EchodexV3FlashCallback`
    function initFlash(FlashParams memory params) external {
        PoolAddress.PoolKey memory poolKey =
            PoolAddress.PoolKey({token0: params.token0, token1: params.token1, fee: params.fee1});
        IEchodexV3Pool pool = IEchodexV3Pool(PoolAddress.computeAddress(deployer, poolKey));
